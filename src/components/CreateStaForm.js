...
const CreateStaffForm = () => {
  console.log("addStaffSchema", addStaffSchema)
  ...
  const { errors, values, handleBlur, handleChange, handleSubmit, isSubmitting } = useFormik({
    initialValues: {
      display_name: "",
      password: "",
      username: "",
    ...
    onSubmit: (values, actions) => {
      ...
      const result = dispatch(createStaffAction(values))
      result.then((res) => {
        console.log(res)
      }).catch((error) => {
        console.log(error)
    ...
    <div className='w-full mt-8'>
      <form className='w-full' onSubmit={handleSubmit}>
        ...
        <div className='flex justify-center mt-8'>
          <button type='submit' className='bg-black py-2 px-2 cursor-pointer rounded-sm text-white' disabled={isSubmitting}>Create new staff account</button>
        </div>
